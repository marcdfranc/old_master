...
/**
 * Servlet implementation class ContratoUpload
 */
public class ContratoUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
    
    public ContratoUpload() {
        super();        
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	out.print("../error_page.jsp?errorMsg=Arquivo enviado com sucesso!&lk=application/cliente_fisico.jsp");
    	out.close();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String href = "../error_page.jsp?errorMsg=Arquivo enviado com sucesso!&lk=application/cliente_fisico.jsp";
		if (FileUpload.isMultipartContent(request)) {
			...
			try {
				 ...
				 while (iterator.hasNext()) {
					...
					} else if (item.getFieldName().trim().equals("idUsuario")) {
				...
				insertImage(fileData, nomeArquivo, this.getServletContext().getRealPath("/application/upload/ctrs"));				
			} catch (Exception e) {
				e.printStackTrace();
				if (transaction != null && transaction.isActive()) {
					transaction.rollback();
				}
				href = "../error_page.jsp?errorMsg=Não foi possível enviar o arquivo devido a um erro interno!&lk=application/cliente_fisico.jsp";
				return;
			} finally {
	...
	private void save(Long idUsuario, Transaction transaction, Session session) throws Exception {
		session = HibernateUtil.getSession();
		transaction = session.beginTransaction();
		Usuario usuario = (Usuario) session.load(Usuario.class, idUsuario);
		usuario.setDocDigital("s");
