...
/**
 * Servlet implementation class ContratoSocialUpload
 */
public class ContratoSocialUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private String href;
    
    public ContratoSocialUpload() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		href = "../error_page.jsp?errorMsg=Arquivo enviado com sucesso!&lk=application/unidade.jsp";
		if (FileUpload.isMultipartContent(request)) {
			...
			try {
				 ...
				 while (iterator.hasNext()) {
					...
					} else if (item.getFieldName().trim().equals("idUnidade")) {
				...
				insertImage(fileData, nomeArquivo, this.getServletContext().getRealPath("/application/upload/contratos_sociais"));															
			} catch (Exception e) {				
				e.printStackTrace();
				if (transaction != null && transaction.isActive()) {
					transaction.rollback();
				}
				href = "../error_page.jsp?errorMsg=Não foi possível enviar o arquivo devido a um erro interno!&lk=application/unidade.jsp";
				return;
			} finally {
	...
	private void save(Long idUnidade, Transaction transaction, Session session) throws Exception {
		session = HibernateUtil.getSession();
		transaction = session.beginTransaction();
		Unidade unidade = (Unidade) session.load(Unidade.class, idUnidade);
		unidade.setDocDigital("s");
