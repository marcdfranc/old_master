...
/**
 * Servlet implementation class LogoCarteirinhaUpload
 */
public class LogoCarteirinhaUpload extends HttpServlet {
			...
			try {
				 ...
				 while (iterator.hasNext()) {
					...
					if ((!item.isFormField()) && (item.getName().length() > 0) && id > 0) {
						insertImage(item, id, this.getServletContext().getRealPath("/report/logo_carteirinha"));															
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void insertImage(FileItem fileIten, Long id, String path) throws FileNotFoundException, IOException{		
		...
		//admin.createThumb(this.getServletContext().getRealPath("/report/logo_carteirinha"), "thumb" + id, extension);
	}
	...
	private boolean saveImage(Long id) {
		...
		try {
			...
			unidade.setLogoCarteirinha("logo_carteirinha/logo" + id + "." + extension);
			unidade.setThumbCarteirinha("../report/logo_carteirinha/thumb" + id + "." + extension);
