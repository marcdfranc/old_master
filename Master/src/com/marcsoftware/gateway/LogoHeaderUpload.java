...
/**
 * Servlet implementation class LogoHeaderUpload
 */
public class LogoHeaderUpload extends HttpServlet {
			...
			try {
				 ...
				 while (iterator.hasNext()) {
					...
					if ((!item.isFormField()) && (item.getName().length() > 0) && id > 0) {
						insertImage(item, id, this.getServletContext().getRealPath("/report/logo"));															
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void insertImage(FileItem fileIten, Long id, String path) throws FileNotFoundException, IOException{		
		...
		admin.createThumb(this.getServletContext().getRealPath("/report/logo"), "thumb" + id, extension);
	}
	...
	private boolean saveImage(Long id) {
		...
		try {
			...
			unidade.setLogo("../report/logo/logo" + id + "." + extension);
			unidade.setThumb("../report/logo/thumb" + id + "." + extension);
