...
/**
 * Servlet implementation class BorderoUpload
 */
public class BorderoUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private String href;
  
    public BorderoUpload() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		href = "../error_page.jsp?errorMsg=Arquivo enviado com sucesso!&lk=application/profissionais.jsp";
		if (FileUpload.isMultipartContent(request)) {
			...
			try {
				 ...
				 while (iterator.hasNext()) {
					...
					}  else if (item.getFieldName().trim().equals("idBordero")) {
				...
				insertImage(fileData, nomeArquivo, this.getServletContext().getRealPath("/application/upload/borderos"));															
			} catch (Exception e) {				
				e.printStackTrace();
	...
	private void save(Long idBordero, Transaction transaction, Session session) throws Exception {
		session = HibernateUtil.getSession();
		transaction = session.beginTransaction();
		BorderoProfissional bordero = (BorderoProfissional) session.load(BorderoProfissional.class, idBordero);
		bordero.setDocDigital("s");
		session.update(bordero);
		transaction.commit();
	}
