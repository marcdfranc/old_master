...
/**
 * Servlet implementation class OrcamentoUpload
 */
public class OrcamentoUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
private String href;
    
    public OrcamentoUpload() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (FileUpload.isMultipartContent(request)) {
			...
			try {				
				...
				insertImage(fileData, nomeArquivo, this.getServletContext().getRealPath("/application/upload/orcamentos"));
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void save(Long idOrcamento, Transaction transaction, Session session) throws Exception {
		...
		orcamento.setDocParcelaDigital("s");
