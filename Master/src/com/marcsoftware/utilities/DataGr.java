/**@Autor: Marcelo de Oliveira Franciso
 * @Cliente: Master Odontologia & Saúde
 * @data_Criação: 12/04/2008 * 
 */ 
/*Uso do data Grid 	
   **********@Instancias de objetos***********   
   unidade= unidadeDAO.getUnidadeList(null); 
	DataGrid dataGrid= new DataGrid(1);
	
	*******Montagem do Cabeçalho  Grid*********	
	dataGrid.addColum("10", "Código");
	dataGrid.addColum("40", "Razão Social");
	dataGrid.addColum("40", "Fantasia");
	dataGrid.addColum("10", "Tipo Contrato");
		
	*******Montagem das Linhas da grid*********	
	dataGrid.mountHeader();
	for (int i=0 ; i < dataGrid.getSize(); i++) {
		dataGrid.addData(String.valueOf(unidade.get(i).getCodigo()));
		dataGrid.addData(unidade.get(i).getRazaoSocial());
		dataGrid.addData(unidade.get(i).getFantasia());
		dataGrid.addData(String.valueOf(unidade.get(i).getType()));
		dataGrid.mountRow();
	}
	...
	**********Printando Grid na tela**********
	out.print(dataGrid.getTable());
 */
...
public class DataGr {
	...
	public DataGr(String href, boolean target) {
		this.href= href;
		this.header= "<div id=\"gridContent\" class=\"dataGrid\"> " + 
			"<table id=\"dg\" cellpadding=\"0\" cellspacing=\"0\" class=\"lastGrid\"> " +
			"<thead class=\"headerGrid\"><tr> ";
