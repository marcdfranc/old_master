...
<%@page import="com.marcsoftware.database.Vigencia"%>
<%@page import="com.marcsoftware.database.EmpresaSaude"%><html xmlns="http://www.w3.org/1999/xhtml">
	...
	if (isEdition) {
		...
		empresa = (EmpresaSaude) sess.get(EmpresaSaude.class, orcamentoIten.get(0).getId().getOrcamento().getPessoa().getCodigo());
...
<head>
	...
	<link rel="StyleSheet" href="../js/jquery/theme/ui.all.css" type="text/css" />	
	...
	<script type="text/javascript" src="../js/comum/cadastro_orcamento_emp.js" ></script>
...
<body onload="loadPage(<%= isEdition %>)">
	...
	<jsp:include page="../inc/menu.jsp">
		<jsp:param name="currentPage" value="orcamentoEmp"/>
	</jsp:include>
	<div id="centerAll">
		...
		<div id="formStyle">
			<form id="formPost" name="formPost" method="post" action="../CadastroOrcamentoEmp" onsubmit="return validForm(this)">
				...
				<div id="geralDate" class="alignHeader" >
					...
					<%if (isEdition) {%>
						<div id="abaMenu">
							...
							<%if (session.getAttribute("perfil").toString().trim().equals("a")
								...
								<div class="aba2">
									<a href="cadastro_parcela.jsp?id=<%= orcamentoIten.get(0).getId().getOrcamento().getCodigo()%>&state=o">Parcelamento Operacional</a>	
								</div>
							<%}%>							
						</div>
					<%} else {%>
						<div id="abaMenu">
							<div class="aba2">
								<a href="orcamento_empresa.jsp?id=-1">Orçamentos</a>	
							</div>							
							<div class="sectedAba2">
								<label>></label>	
							</div>							
					...
					<div class="topContent">
						<div class="textBox" id="unidadeId" name="unidadeId">
							...
							<select id="unidadeIdIn" name="unidadeIdIn" <% if (isEdition) {%> disabled="disabled" <%}%> >
						...
						<div id="unidade" class="textBox" style="width: 280px;" >
							<label>Unidade</label><br/>
							<input id="unidadeIn" name="unidadeIn" type="text" style="width: 280px;" <%
								if ((!isEdition) && unidadeList.size()== 1) {
									out.print("value=\"" + Util.initCap(unidadeList.get(0).getFantasia()) + "\"");
								} else if(isEdition) {
									out.print("value=\"" + 
											Util.initCap(orcamentoIten.get(0).getTabela().getUnidade().getFantasia()) +	"\"");
								}%> class="required" enable="false" onblur="genericValid(this);" readonly="readonly" />
						</div>
						...
						<div id="setor" class="textBox" style="width: 105px">
							...
							<select type="select-multiple" <% if (isEdition) {%> disabled="disabled" <%}%> id="setorIn" name="setorIn" >
								<%if (isEdition) {%>
									<option value="<%= empresa.getEspecialidade().getSetor() %>"><%= descSetor %></option>
								<%} else {%>
									<option value="">Selecione</option>
						...
						<div id="empId" class="textBox" style="width: 262px;">
							<label>Empresa de Saúde</label><br/>
							<select id="empIdIn" name="empIdIn" <% if (isEdition) {%> disabled="disabled" <%}%> style="width: 262px">
								<%if (isEdition) {%>								
									<option value="<%= empresa.getCodigo() + "@" + empresa.getConselhoResponsavel()%>"><%= empresa.getFantasia()%></option>
								<%} else {%> 
									<option value="" >Selecione</option>
								<%}%>
							</select>
						...
						<div id="conselho" class="textBox" style="width: 150px" >
							...
							<input id="conselhoIn" name="conselhoIn" type="text" style="width: 150px;" value="<%=(isEdition)? empresa.getConselhoResponsavel(): "" %>" class="required" enable="false" onblur="genericValid(this);" readonly="readonly" />
						</div>						
				...
				<%if (isEdition) {%>
					<div class="topButtonContent">
						<div class="formGreenButton">
							<input id="impOrc" name="impOrc" class="greenButtonStyle" type="button" value="Imprimir" onclick="printOrcamento()"/>
						</div>
					</div>
				<%}%>
				<div id="mainContent">					
