...
<%@page import="com.marcsoftware.database.Vigencia"%><html xmlns="http://www.w3.org/1999/xhtml">
	...
	<jsp:useBean id="orcamento" class="com.marcsoftware.database.Orcamento"></jsp:useBean>
	...
	if (isEdition) {
		orcamento = (Orcamento) sess.get(Orcamento.class, Long.valueOf(request.getParameter("id")));
		...
		profissional = (Profissional) sess.get(Profissional.class, orcamento.getPessoa().getCodigo());
		...
		if (haveParcela.equals("s")) {
			...
			query.setLong("orcamento", orcamento.getCodigo());
...
<body onload="loadPage(<%= isEdition %>)">
	...
	<div id="centerAll">
		...
		<div id="formStyle">
			<input id="haveDoc" name="haveDoc" type="hidden" value="<%=(isEdition)? orcamento.getDocDigital() : "" %>"/>
			<form id="formPost" name="formPost" method="post" action="../CadastroOrcamento" onsubmit="return validForm(this)">
				...
				<div>
					<input id="codOrcamento" name="codOrcamento" type="hidden" value="<%=(isEdition)? orcamento.getCodigo(): "" %>" />
					<input id="userIdIn" name="userIdIn" type="hidden" value="<%= (isEdition) ? orcamento.getUsuario().getCodigo() : "" %>" />
					<%if (isEdition) { %>
				...
				<div id="geralDate" class="alignHeader" >
					...
					<%if (isEdition) {%>
						<div id="abaMenu">
							<div class="aba2">
								<a href="cadastro_cliente_fisico.jsp?state=1&id=<%= orcamento.getUsuario().getCodigo()%>">Cadastro</a>
							</div>
							<div class="sectedAba2">
								<label>></label>	
							</div>
							<div class="aba2">
								<a href="anexo_fisico.jsp?id=<%=orcamento.getUsuario().getCodigo()%>">Anexo</a>
							</div>
							<div class="sectedAba2">
								<label>></label>	
							</div>
							<div class="aba2">
								<a href="mensalidade.jsp?id=<%= orcamento.getUsuario().getCodigo()%>">Mensalidades</a>
							</div>
							<div class="sectedAba2">
								<label>></label>	
							</div>
							<div class="aba2">
								<a href="orcamento_empresa.jsp?id=<%= orcamento.getUsuario().getCodigo()%>">Orçamentos de Empresas</a>
							</div>							
							<div class="sectedAba2">
								<label>></label>	
							</div>
							<div class="aba2">
								<a href="orcamento.jsp?id=<%= orcamento.getUsuario().getCodigo()%>">Orçamentos de Profissionais</a>
							</div>
							<div class="sectedAba2">
								<label>></label>	
							</div>
							...
							<div class="aba2">
								<a href="cadastro_parcela.jsp?id=<%= orcamento.getCodigo()%>">Parcelamento</a>	
							</div>
							<%if (session.getAttribute("perfil").toString().trim().equals("a")
								...
								<div class="aba2">
									<a href="cadastro_parcela.jsp?id=<%= orcamento.getCodigo()%>&state=o">Parcelamento Prof.</a>	
								</div>
							<%}%>							
						</div>
					<%} else {%>
					...
					<div class="topContent">
							...
							<select id="unidadeIdIn" name="unidadeIdIn" <% if (isEdition) {%> disabled="disabled" <%}%> style="width: 383px" >
								...
								<%if (!isEdition && unidadeList.size()== 1) {
									...
									for(Unidade un: unidadeList) {
										if (isEdition && orcamentoIten.size() > 0 && orcamentoIten.get(0).getTabela().getUnidade().equals(un)) {
											out.print("<option selected=\"selected\" value=\"" + 
													un.getFantasia() + "@" + 
													un.getCodigo() + "\">" + 
													un.getReferencia() + " - " +
						...
						<div id="username" class="textBox" style="width: 150px;">
							...
							<input id="usernameIn" name="usernameIn" class="required" type="text" "readonly="readonly" style="width: 150px;" value="<%=(isEdition)? orcamento.getLogin().getUsername() : session.getAttribute("username") %>"/>
						</div>
						<div id="cadastro" class="textBox" style="width: 73px;">
							<label>Cadastro</label><br/>
							<input id="cadastroIn" name="cadastroIn" class="required" type="text" <%if(isEdition) {%> "readonly="readonly" <%}%> style="width: 73px;" value="<%=(isEdition)? Util.parseDate(orcamento.getData(), "dd/MM/yyyy") : Util.getToday() %>" <%if(!isEdition) {%> onkeydown="mask(this, dateType);" <%}%>/>
						</div>
						<div id="setor" class="textBox" style="width: 130px">
							<label>Atividade</label><br/>
							<select type="select-multiple" <% if (isEdition) {%> disabled="disabled" <%}%> id="setorIn" name="setorIn" style="width: 130px" >
								<%if (isEdition) {%>
						...
						<div id="profId" class="textBox" style="width: 238px;">
							<label>Profissional</label><br/>
							<select id="profIdIn" name="profIdIn" <% if (isEdition) {%> disabled="disabled" <%}%> style="width: 238px">
								<%if (isEdition) {%>								
						...
						<div class="textBox" id="orcId" style="width: 78px">
							...
							<input id="orcIdIn" name="orcIdIn" type="text" style="width: 73px;" value="<%=(isEdition)? orcamento.getCodigo(): "" %>" readonly="readonly"/>
						</div>
						<div class="textBox" id="userId" style="width: 80px">
							<label>CTR</label><br/>
							<input id="ctr" name="ctr" type="text" style="width: 80px;" value="<%=(isEdition)? Util.initCap(orcamento.getUsuario().getReferencia()): "" %>" class="required" onkeydown="mask(this, onlyNumber);" onblur="genericValid(this);" <%if(isEdition) { out.print("readonly=\"readonly\""); }%> />
						</div>
						<div id="usuario" class="textBox" style="width: 275px;" >
							<label>Titular</label><br/>
							<input id="usuarioIn" name="usuarioIn" type="text" style="width: 270px;" value="<%=(isEdition)? Util.initCap(orcamento.getUsuario().getNome()): "" %>" enable="false" readonly="readonly" />
						</div>
						<div class="textBox" id="dependenteId" style="width: 256px">
							...
							<select id="dependenteIdIn" name="dependenteIdIn" style="width: 256px;" <%if (isEdition) out.print("disabled=\"disabled\""); %>>
								<option value="" >Selecione</option> 								
								<%if (isEdition && (orcamento.getDependente() != null)) {
									out.print("<option selected=\"selected\" value=\"" +
											orcamento.getDependente().getCodigo() + "\">" + 
											Util.initCap(orcamento.getDependente().getNome()) + 
											"</option>");
								}%>
							</select>
						</div>
				...
				<%if (isEdition) {%>
					<div class="topButtonContent">
						<div class="formGreenButton">
							<input  class="greenButtonStyle" type="button" value="Imprimir" onclick="printOrcamento()"/>
						</div>
						<div class="formGreenButton">
					...
					<div id="itensOrcamento" class="bigBox" >
						...
						<div id="tabela" class="textBox">
							...
							<select type="select-multiple" id="tabelaIn" name="tabelaIn" class="required" onblur="genericValid(this);" style="width: 315px" >
								<option value="">Selecione</option>
								<%if(isEdition && orcamentoIten.size() > 0) {
					...
					<div id="dataGrid">
						...
						if (isEdition) {
							for (ItensOrcamento iten: orcamentoIten) {
								...
								case 'a':
									dataGrid.addData("rowSetor", "Administrativa");
									break;
									
								case 'e':
									dataGrid.addData("rowSetor", "Estética");
									break;
								}						
