				...
				<div id="geralDate" class="alignHeader" >
					...
					<div class="topContent">						
						...
						<div id="unidade" class="textBox" style="width: 280px;">
							...
							<input id="unidadeIn" name="unidadeIn" type="text" style="width: 280px;" value="<%
						...
						<div id="setor" class="textBox" style="width: 221px">
							<label>Atividade</label><br/>
							<select type="select-multiple" id="setorIn" name="setorIn" class="required" onblur="genericValid(this);" <%if(isEdition) out.print("disabled=\"disabled\""); %> style="width: 221px">
								<option value="">Selecione</option>
								<option <%if (isEdition 
										&& servico.getEspecialidade() != null
										&& servico.getEspecialidade().getSetor().equals("o")) {
									out.print("selected=\"selected\"");
								}%> value="o">Odontológica</option>								
								<option <%if (isEdition
										&& servico.getEspecialidade() != null
										&& servico.getEspecialidade().getSetor().equals("l")) {
									out.print("selected=\"selected\"");
								}%> value="l">Laboratorial</option>
								<option <%if (isEdition 
										&& servico.getEspecialidade() != null
										&& servico.getEspecialidade().getSetor().equals("m")) {
									out.print("selected=\"selected\"");
								}%> value="m">Médica</option>
								<option <%if (isEdition 
										&& servico.getEspecialidade() != null
										&& servico.getEspecialidade().getSetor().equals("h")) {
									out.print("selected=\"selected\"");
								}%> value="h">Hospitalar</option>								
								<option value="a" 
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("a")) {
										out.print("selected=\"selected\"");
									}%>>Administrativa</option>									
								<option value="e" 
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("e")) {
										out.print("selected=\"selected\"");
									}%>>Estética</option>
								<option value="n" <%if (isEdition 
										&& servico.getEspecialidade() != null
										&& servico.getEspecialidade().getSetor().trim().equals("n")) {
										out.print("selected=\"selected\"");
									}%>>Ensino</option>						
								<option value="p"
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("p")) {
										out.print("selected=\"selected\"");
									}%>>Prest. de Serviços</option>						
								<option value="j" 
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("j")) {
										out.print("selected=\"selected\"");
									}%> >Jurídica</option>						
								<option value="u"
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("u")) {
										out.print("selected=\"selected\"");
									}%>>Automobilistica</option>
								<option value="c" 
									<%if (isEdition 
											&& servico.getEspecialidade() != null
											&& servico.getEspecialidade().getSetor().trim().equals("c")) {
										out.print("selected=\"selected\"");
