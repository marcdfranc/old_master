...
<head>
	...
	<script type="text/javascript" src="../js/comum/insumo.js" ></script>
...
<body onload="load()">
	<div id="cadastroWindow" class="removeBorda" title="Cadastro de Produto Serviço" style="display: none;">
		<form id="formObs" onsubmit="return false;">		
			<label>Ramo</label><br/>
			<select id="ramoCad" name="ramoCad" class="textDialog ui-widget-content ui-corner-all">
				<option value="">Selecione</option>						
				<%for(Ramo ramo: ramoList) {
						out.print("<option value=\"" + ramo.getCodigo() + "\">" + 
								Util.encodeString(ramo.getDescricao(), "ISO-8859-1", "UTF-8")  + "</option>");
				}%>
			</select>
			<label>Descrição</label><br/>
			<input id="descricaoCad" name="descricaoCad" type="text" class="required textDialog ui-widget-content ui-corner-all"  />
			
			<label>Tipo</label><br/>
			<select id="tipoCad" name="tipoCad" class="textDialog ui-widget-content ui-corner-all">
			...
			<select id="statusCad" name="statusCad" class="textDialog ui-widget-content ui-corner-all">
				<option value="a">Ativo</option>								
				<option value="d">Descontinuado</option>
			</select>
						
			<label>Cod. Unid.</label><br/>
			<select id="unidadeCad" name="unidadeCad" class="textDialog ui-widget-content ui-corner-all">
	...
	<%if ((request.getParameter("origem").equals("prest"))) {%>
		...
		<div id="formStyle">
			<form id="unit" method="get" onsubmit="return search()">
				...
				<div class="topButtonContent">					
					...
					<div class="formGreenButton">
						<input id="buscar" name="buscar" class="greenButtonStyle" onclick="cadastro();" type="button" value="Salvar"/>
					</div>
				</div>
				<div id="mainContent">
					...
					<div id="dataGrid">
						...
						dataGrid.addColum("38" ,"Ramo");
						dataGrid.addColum("38" ,"Descrição");
						dataGrid.addColum("5" ,"Unid.");
						dataGrid.addColum("4" ,"Tipo");
						...
						for(Insumo insumo: insumoList) {
							...
							if(insumo.getTipo().trim().equals("s")) {
								dataGrid.addData("tipo" + insumo.getCodigo(), "Serviço", false);
							} else {
								dataGrid.addData("tipo" + insumo.getCodigo(), "Produto", false);
							}
