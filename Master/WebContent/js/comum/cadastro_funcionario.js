...
$(document).ready(function(){
	loadPage = function(isEd) {
		...
		conta.addCol("Titular", "40", "rowPrincipal");		
		conta.addCol("Banco", "25", "rowBank");
		conta.addCol("Agencia", "10", "rowAgency");
		conta.addCol("Conta", "25", "rowCont");
	...
	addRowContact= function(){
		if (($('#descricaoIn').val() != "") && ($('#tipoContato').val() != "Selecione")) {
			if (isEdition) {
	...
	confirmarSenha= function(object) {
		if (object.className == "greenButtonStyle") {
			if ($('#senhaIn').val() != $('#senhaConfirmIn').val()) {
				showErrorMessage("As duas senhas devem ser iguais para que ocorra alteração!", 
				"Erro", 180, 600, true);
			} else {
	...
	loadFile = function() {
		...
		window.open('upload.jsp?id=\'' + $('#codFuncionario').val() + '\'', 'nova', 'width= 600, height= 200, top= ' + top + ', left= ' + left);
	}	
});
